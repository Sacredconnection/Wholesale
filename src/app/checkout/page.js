                  ...
                  {step < STEPS.length - 1 ? (
                    ...
                    <>
                      <button
                        type="button"
                        onClick={submitOrder}
                        disabled={isSubmitting || !confirmed || !minimumStatus.meetsMinimum}
                ...
                {!minimumStatus.meetsMinimum && (
                  <div className="mt-2 rounded-sm border border-yellow-400/25 bg-yellow-400/10 px-3 py-2.5 text-[10px] font-semibold leading-relaxed text-yellow-300">
                    {minimumStatus.type === "weight" ? (
                      <>
                        Minimum wholesale order: {minimumStatus.required} g.
                        Add {Math.ceil(minimumStatus.remaining)} g more before submitting.
                      </>
                    ) : (
                      <>
                        Minimum wholesale order: ${minimumStatus.required.toFixed(2)}.
                        Add ${minimumStatus.remaining.toFixed(2)} more before submitting.
                      </>
                    )}
                  </div>
                )}
